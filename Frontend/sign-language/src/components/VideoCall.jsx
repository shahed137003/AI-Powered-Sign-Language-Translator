...
const VideoCall = ({ 
  ...
  const startAICameraAndWS = async () => {
    ...
    const aiWs = new WebSocket(getWsUrl(8001, "/ws/sign-to-text"));
  ...
  const handleAcceptPrediction = () => {
    if (lowConfPrediction && aiSocketRef.current?.readyState === WebSocket.OPEN) {
      aiSocketRef.current.send(JSON.stringify({
        type: "add_word",
    ...
    <AnimatePresence>
      ...
      >
        <div className="relative w-full h-full">
          ...
          <AnimatePresence>
            {isAiEnabled && lowConfPrediction && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 20 }}
                className="absolute bottom-[20rem] left-1/2 transform -translate-x-1/2 bg-[#2d2208]/90 backdrop-blur-md rounded-2xl p-4 border border-yellow-500/50 shadow-2xl max-w-md w-full mx-4 z-40"
              >
                <div className="flex flex-col gap-3">
                  <div>
                    <span className="text-xs font-bold uppercase tracking-wider text-yellow-400">
                      ⚠️ Low Confidence Sign Detected
                    </span>
                    ...
                    <p className="text-xs text-gray-300 mt-1">
                      Did you mean this? Accept to add to your sign buffer or reject to skip.
                    </p>
                  </div>
