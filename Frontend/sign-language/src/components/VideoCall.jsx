...
const VideoCall = ({ 
  ...
  // AI Translation states – updated to match Translate page
  const [isAiEnabled, setIsAiEnabled] = useState(false);
  const [aiGlossText, setAiGlossText] = useState("");      // gloss words (sentence_buffer)
  const [aiEnglishText, setAiEnglishText] = useState("");  // LLM translation
  const [aiConfidence, setAiConfidence] = useState(0);
  const [isAiRecording, setIsAiRecording] = useState(false);
  const [isAiLoading, setIsAiLoading] = useState(false);
  const isAiEnabledRef = useRef(false);
  
  // Refs for WebRTC (unchanged)
  ...
  // AI Refs – updated (no MediaPipe Camera class)
  const aiSocketRef = useRef(null);
  const aiHolisticRef = useRef(null);
  const aiVideoRef = useRef(null);          // hidden video element for AI processing
  const aiFrameRequestRef = useRef(null);
  const aiLastFrameTime = useRef(0);
  const AI_FRAME_INTERVAL_MS = 33;          // ~30 fps
  ...
  // ==========================================
  // Load MediaPipe scripts (without camera_utils)
  // ==========================================
  ...
  // ==========================================
  // AI Translation Setup (custom camera loop)
  // ==========================================
  const startAICameraAndWS = async () => {
    ...
    if (!localStream) {
      console.error("No local stream available for AI");
      return;
    }
    ...
    aiWs.onmessage = (event) => {
      try {
        ...
        if (data.status === "collecting") {
          ...
          // Update gloss buffer (sentence_buffer) and LLM translation
  ...
  const initializeAIMediaPipe = () => {
    ...
    holistic.onResults((results) => {
      ...
      if (aiSocketRef.current?.readyState === WebSocket.OPEN && isAiEnabledRef.current) {
        ...
        aiSocketRef.current.send(JSON.stringify({
          type: "keypoints",
          data: keypoints,
          hands_visible: handsVisible   // crucial for backend recording logic
        }));
      }
    });
    ...
    // Create a hidden video element for AI processing (not the main video)
    ...
    // Custom frame loop (throttled to 30 fps)
    const processFrame = async (now) => {
      if (!isAiComponentMounted.current || !aiHolisticRef.current || !aiVideoRef.current || !isAiEnabledRef.current) return;
      if (now - aiLastFrameTime.current >= AI_FRAME_INTERVAL_MS) {
        aiLastFrameTime.current = now;
        if (aiVideoRef.current.readyState >= 2) {
          try {
            await aiHolisticRef.current.send({ image: aiVideoRef.current });
          } catch (e) {
    ...
    setIsAiLoading(false);
    isAiStarting.current = false;
    console.log("🎥 AI camera started (custom loop, 30 fps)");
  };
    ...
    <AnimatePresence>
      ...
      >
        <div className="relative w-full h-full">
          ...
          {/* AI Translation Overlay (two panels: gloss and translation) */}
