...
const originalMidnightBlue = `html[data-theme="midnight-blue"] {
  ...
  --theme-primary-bg-1: #02040a;
  --theme-primary-bg-2: #0f0d2a;
  --theme-primary-bg-3: #1c1955;
  --theme-primary-bg-4: #02040a;
  --theme-primary-bg-5: #0f0d2a;
}
`;
...
files.forEach(file => {
  if (fs.existsSync(file)) {
    ...
    const oldStr = /'midnight-blue': \['#3B82F6', '#2563EB', '#1D4ED8', '#1E40AF', '#60A5FA'\]/g;
    const newStr = `'midnight-blue': ['#6366F1', '#4F46E5', '#4338CA', '#3730A3', '#818CF8']`;
    
    if (content.match(oldStr)) {
      content = content.replace(oldStr, newStr);
      fs.writeFileSync(file, content, 'utf8');
      console.log('Reverted particles in ' + file);
    }
  }
});
