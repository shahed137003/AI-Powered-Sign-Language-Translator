...
const filePath = 'd:/GP/Frontend/sign-language/src/components/ResetPassword.jsx';
let content = fs.readFileSync(filePath, 'utf8');

// 1. Add missing import for useTheme
if (!content.includes('import { useTheme } from "../context/ThemeContext"')) {
  content = content.replace(/import { useLocation, useNavigate, Link } from 'react-router-dom';/, `import { useLocation, useNavigate, Link } from 'react-router-dom';\nimport { useTheme } from "../context/ThemeContext";`);
}

// 2. Add themeColor hook inside the component
if (!content.includes('const { themeColor } = useTheme();')) {
  content = content.replace(/export default function ResetPassword\(\) \{/, `export default function ResetPassword() {\n  const { themeColor } = useTheme();`);
}
...
console.log('Fixed ResetPassword.jsx theme');
