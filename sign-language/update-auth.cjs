...
const replacement = `            {/* Premium Feature Cards */}
              ...
              {[
                ...
                >
                  ...
                  <div className="relative h-full p-4 sm:p-5 flex flex-col items-center text-center justify-center rounded-3xl bg-white/70 dark:bg-white/5 backdrop-blur-xl border border-primary-200/50 dark:border-primary-500/20 shadow-lg shadow-primary-100/20 dark:shadow-primary-900/20 group-hover:bg-white/90 dark:group-hover:bg-white/10 transition-colors duration-300 overflow-hidden">
                    ...
                    {/* Big background icon */}
                    <feature.icon className="absolute -bottom-4 -right-4 text-7xl sm:text-8xl text-gray-400 dark:text-gray-500 opacity-[0.06] group-hover:opacity-[0.12] group-hover:scale-110 group-hover:-rotate-12 transition-all duration-500 pointer-events-none" />
                    
                    <div className={\`p-3 sm:p-4 rounded-2xl bg-gradient-to-br \${feature.color} shadow-lg mb-3 group-hover:-translate-y-1 transition-transform duration-300\`}>
                      <feature.icon className="text-2xl sm:text-3xl text-white drop-shadow-md group-hover:scale-110 transition-transform duration-300" />
                    </div>
                    
                    <h3 className="font-bold text-gray-900 dark:text-white mb-1 text-xs sm:text-sm z-10">{feature.title}</h3>
                    <p className="text-gray-600 dark:text-gray-300 text-[10px] sm:text-xs font-medium z-10 break-words w-full">{feature.description}</p>
                  </div>
                </motion.div>
              ))}
            </div>`;
...
function processFile(filePath) {
  ...
  let endIndex = content.indexOf('{/* Quick Stats */}');
...
processFile('d:/GP/Frontend/sign-language/src/components/Login.jsx');
processFile('d:/GP/Frontend/sign-language/src/components/Register.jsx');
