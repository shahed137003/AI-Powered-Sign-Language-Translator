...
class ASLPipeline:
    ...
    def end_recording(self):
        """
        Force ending the current recording and predicting on the collected buffer.
        """
        if self.recording:
            self.recording = False
            buf_len = len(self.buffer)
            print(f"✋ Force stopped recording. Buffer size: {buf_len}")
            ...
            if buf_len > MIN_FRAMES:
                ...
                print(f"⚠️  Gesture too short for force-predict: {buf_len} frames (min {MIN_FRAMES})")

            self.buffer = []
            
        # Even if not actively recording, return the last known state
        return (
            self.last_pred,
            self.last_conf,
            self.sentence_buffer,
    ...
    def add_word(self, word):
        if word and (not self.sentence_buffer or self.sentence_buffer[-1] != word):
