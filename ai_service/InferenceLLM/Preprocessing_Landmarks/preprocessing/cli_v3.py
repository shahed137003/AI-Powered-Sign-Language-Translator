...
def fix_length_to_target(x: np.ndarray, target_len: int):
    """
    Returns:
      x_fixed  -> (target_len,438)
    ...
    if x.ndim != 2 or x.shape[1] != FEATURE_DIM:
        raise ValueError(
            f"Expected shape (T,{FEATURE_DIM}), got {x.shape}"
        )
