...
# class SignToTextService:
#     # New parameters from main_inference
#     TARGET_LEN = 120        # Fixed sequence length (was 157)
#     MIN_FRAMES = 15         # Minimum frames (was 20)
#     COOLDOWN_FRAMES = 12    # Frames to wait after hands disappear
...
#     def add_keypoints(self, keypoints: list, hands_visible: bool = True):
#         """
#         Process keypoints with auto-trigger logic (same as main_inference)
#         Returns prediction dict if ready, otherwise status dict
#         """
#         kp_array = np.array(keypoints)
        
#         # Auto-start recording when hands appear
#         if hands_visible and not self.is_recording:
#             self.is_recording = True
#             self.sequence = []
#             self.silence_counter = 0
...
#         # If recording, add keypoints
#         if self.is_recording:
#             self.sequence.append(kp_array)
            
#             # Reset silence counter if hands visible
#             if hands_visible:
#                 self.silence_counter = 0
#             else:
#                 self.silence_counter += 1
            
#             # Auto-predict when hands are gone for COOLDOWN_FRAMES
#             if self.silence_counter >= self.COOLDOWN_FRAMES:
#                 if len(self.sequence) >= self.MIN_FRAMES:
#                     print(f"Hands gone. Buffer size: {len(self.sequence)}")
#                     result = self.predict_sequence()
#                     self.is_recording = False
#                     return result
#                 else:
#                     # Too short, just reset
#                     print(f"Sequence too short: {len(self.sequence)} frames")
#                     self.is_recording = False
#                     self.sequence = []
#                     return {
#                         "status": "error",
#                         "text": f"Too short ({len(self.sequence)} frames)",
#                         "confidence": 0.0
#                     }
            
#             # Return recording status (for UI progress)
#             progress = min(100, int((len(self.sequence) / self.TARGET_LEN) * 100))
#             return {
#                 "status": "collecting",
...
#         """Run inference on collected sequence (same as main_inference)"""
#         if len(self.sequence) < self.MIN_FRAMES:
#             self.sequence = []
#             return {
#                 "text": f"Too short ({len(self.sequence)} frames)",
...
#             # Preprocess using v3 pipeline (same as main_inference)
#             proc_seq = preprocess_sequence_global(raw_sequence)
...
#             return {
#                 "text": predicted_word,
#                 "confidence": confidence / 100.0,  # Convert to 0-1 for frontend
#                 "status": "success"
#             }
...
#     def force_predict(self):
#         """Force prediction on current sequence (manual stop)"""
#         if len(self.sequence) >= self.MIN_FRAMES:
#             result = self.predict_sequence()
#             self.is_recording = False
#             return result
