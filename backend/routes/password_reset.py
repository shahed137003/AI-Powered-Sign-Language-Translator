...
def forget_password(data: ForgetPasswordSchema, db: Session = Depends(get_db)):
    ...
    sent = send_email(user.email, "Password Reset Code", f"Your reset code is: {code}")
    if not sent:
        raise HTTPException(
            status_code=500,
            detail="Reset code generated but failed to send email. Please check your email address or try again later."
        )
