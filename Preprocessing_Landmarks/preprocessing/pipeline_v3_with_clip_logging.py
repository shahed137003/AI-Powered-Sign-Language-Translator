...
from .hands_with_clip_logging  import translate_hands_before_swap,fix_swap_and_gate_hands, fill_hand_gaps_anchor_relative_tiered, trim_edge_filler_frames
...
def preprocess_sequence_global(
    ...
    # same-side pose anchors in preprocessed space:
    # left  = wrist, pinky, index, thumb
    # right = wrist, pinky, index, thumb
    lpose = pose_xyz[:, [15, 17, 19, 21], :].copy()
    rpose = pose_xyz[:, [16, 18, 20, 22], :].copy()

    # wrist-only arrays kept only if needed elsewhere.
    # Translation, swapping, and filling below all use full same-side pose-anchor block
    lw = lpose[:, 0, :].copy()
    rw = rpose[:, 0, :].copy()
    # mandatory translation stage
    
    translate_hands_before_swap(
     lh, rh, lpose, rpose,
    ...
    if fill_hands:
        fill_hand_gaps_anchor_relative_tiered(
            ...
            anchor_max_err=hand_wrist_max_dist,
            eps=eps,
        )
        fill_hand_gaps_anchor_relative_tiered(
            rh, rpose,
