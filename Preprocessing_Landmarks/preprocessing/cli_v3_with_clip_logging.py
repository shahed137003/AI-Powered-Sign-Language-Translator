...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--no-trim-edge-filler", action="store_true", help="Disable trimming leading/trailing frames where both hands are absent")
    ...
    ap.add_argument("--hand-wrist-max-dist", type=float, default=0.4,
                help="Maximum allowed same-side pose/hand anchor error before applying anchor-based hand translation and fill endpoint rejection. (default: 1.1)")
...
def main() -> None:
    ...
    def run_one(x: np.ndarray, clip_path: str | None = None) -> np.ndarray:
    ...
    if args.input_npy:
        ...
        y = run_one(x, clip_path=args.input_npy)
    ...
    for in_path in files:
        ...
        y = run_one(x, clip_path=str(in_path))
        if args.target_frames is not None:
            try:
                y = fix_length_to_target(y, args.target_frames)
            except ValueError as e:
