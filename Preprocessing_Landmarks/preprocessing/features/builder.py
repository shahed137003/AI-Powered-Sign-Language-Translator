...
def build_features(seq: np.ndarray) -> np.ndarray: 
    ...
    #velocity
    v = np.zeros_like(x) #create an array same shape as x and filled with all 0s
    for t in range(1, T):
        if valid[t] and valid[t - 1]:
            v[t] = x[t] - x[t - 1]
    #this loop is used to comptue velocity which is position now - position in prev frame
    #velocity is vector so has both magnitude and direction  
    #to compute velocity the two frames must be valid frames, if any of them not valid so wont compute it between these 2 frames

    #acceleration
    a = np.zeros_like(x) #create an array same shape as x and filled with 0s
    for t in range(1, T):
        if valid[t] and valid[t - 1]:
            a[t] = v[t] - v[t - 1]
    #this loop is used to compute acceleration which is velocty now - velocity in prev frame
    #to compute acceleration the two frames must be valid frames, if any of them not valid so wont compute it between these 2 frames
    #acceleration detects uniform vs sudden changes, if motion is all same speed and same direction so acceleration almost zero so thats steady motion
    #but if acceleration is high so there is sudden change in velocity which indicates strong movement
    #so velocity detects direction and magnitude of speed but acceleration detects if motion is steady or strong chnages

    a = np.tanh(a)
    #tiny landmark noise can lead to huge acceleration spike, this leads to model overfitting on noise
    #tanh takes values from any range -inf to inf and puts them in the range of -1 to 1
    #so small values are kept unchanges but large values are clipped because they are mostly noise
    ...
    # Motion direction
    direction = v / safe_norm(v) # we define the velocity vector by its norm to get the velocity direction
    #notice that we used safe_norm function we created because if norm here is 0 so this is division by 0 so error
    #so thats the use of safe_norm function, it avoids division by 0 or Nans
    ...
    features = np.concatenate([
        x, #x is original landmarks we receive from the preprocessing which is (T,438)
        v, #v is velocity vector with magnitude and direction and is used to represent motion between frames, v[t]=x[t]-x[t-1]
        a, #a is acceleration which is change in speed (smooth or sharp)
        ...
        direction, #direction is velocity vector / magnitude so it keeps direction only
